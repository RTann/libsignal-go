...
func TestGroupNoSendSession(t *testing.T) {
	...
	distributionID := distributionid.MustParse("d1d1d1d1-7000-11eb-b32a-33b8a8a487a6")

	aliceStore := testInMemProtocolStore(t, random)

	groupSession := &session.GroupSession{
		Sender:         senderAddress,
		SenderDistID:   distributionID,
		SenderKeyStore: aliceStore.GroupStore(),
	}

	_, err := groupSession.EncryptMessage(ctx, random, []byte("space camp?"))
	assert.Error(t, err)
}
