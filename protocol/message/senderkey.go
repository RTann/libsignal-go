...
func (s *SenderKey) DistributionID() distributionid.ID {
...
func NewSenderKeyDistributionFromBytes(bytes []byte) (*SenderKeyDistribution, error) {
	...
	distributionID, err := distributionid.ParseBytes(message.GetDistributionUuid())
	if err != nil {
		return nil, err
	}
...
func (s *SenderKeyDistribution) DistributionID() distributionid.ID {
	return s.distributionID
}
